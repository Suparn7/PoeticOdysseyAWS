...
const Profile = () => {
            ...
            try {
                if (userInfoData && profileId.slug && userInfoData.userId !== profileId.slug) {
                    ...
                    if (userInfoData) {
                        ...
                        //setFollowers(userInfoData.followers);
                        //setFollowing(userInfoData.following);
                    }
                }
            } catch (e) {
                console.error(e);
        ...
        try {
            ...
            if (profilePic) {
                ...
                if (userInfoData && userInfoData.profilePicUrl) {
                    const key = userInfoData.profilePicUrl.split('.com/')[1]; // Extract the S3 key
    ...
    const handleFollowClick = async () => {
        try {
            ...
            const currentUserId = userInfoData.userId; // Replace with actual current user ID
    ...
    const fetchFollowing = async () => {
        try {
            ...
            let followingData;
            if(profileId?.slug){
                followingData = await dynamoUserInformationService.getFollowing(profileId.slug); // You need to implement this service
            }else{
